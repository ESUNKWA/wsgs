...
export class BoutiqueService {
  ...
  private async resolveDs(structureId?: number): Promise<DataSource> {
    ...
    throw new InternalServerErrorException(
      'Structure requise pour accéder aux boutiques.',
    );
  }

  async create(
    createBoutiqueDto: CreateBoutiqueDto,
    ...
    try {
      ...
      const structureId =
        dtoStructureId ??
        (this.tenantContext.hasContext()
          ? (this.tenantContext.getStructureId() ?? undefined)
          : undefined);
      ...
      const isUnitéCommerciale = !['departement', 'entrepot'].includes(
        typeNouvelle,
      );

      // Compter uniquement les boutiques (pas les dépôts/départements)
      const nbExistantes =
        structureId && isUnitéCommerciale
          ? await repo.count({
              where: {
      ...
      const data = repo.create({
        ...
        is_active: true,
        logo: file
          ? buildTenantFilePath(structureId ?? null, 'logos', file.filename)
          : null,
      });
      ...
      if (structureId && isUnitéCommerciale && nbExistantes === 0) {
        ...
        await this.abonnementService
          .demarrerEssai(structureId)
          .catch(() => null);
        ...
        await this.abonnementService
          .notifierAjoutBoutique(structureId, saved.id, saved.nom)
          .catch(() => null);
      }
  ...
  async toggleActive(
    ...
    if (sid) {
      if (activer) {
        await this.abonnementService
          .notifierActivationBoutique(sid, id)
          .catch(() => null);
      } else {
        await this.abonnementService
          .notifierDesactivationBoutique(sid, id)
          .catch(() => null);
      }
    }
  ...
  async findByStructure(idStructure: string): Promise<Boutique[]> {
    ...
    if (isNaN(id))
      throw new BadRequestException('Identifiant de structure invalide');
  ...
  async update(
    id: number,
    updateBoutiqueDto: UpdateBoutiqueDto,
    file?: Express.Multer.File,
    structureId?: number,
  ) {
    try {
      ...
      if (!boutique) throw new NotFoundException('Boutique inexistante');
      if (file)
        boutique.logo = buildTenantFilePath(
          sid ?? null,
          'logos',
          file.filename,
        );
